...
def _parse_tokens(tokens, reversion=False, version=None):
    ...
    for token_set in token_sets:
        ...
        if ed_type == "QUOTED_STRING":
            ...
            raise InvalidFormat(
                "Could not identify edit descriptor in sequence {0:s}".format(
                    str(token_set)
                )
            )
    ...
    for t0 in get_tokens:
        if t0.type != "LEFT_PARENS":
            ...
            if (len(new_tokens) > 0) and (new_tokens[-1].type in ["NZUINT", "UINT"]):
                ...
                new_tokens.extend(
                    repeat * (_expand_parens(paren_tokens) + [Token("COMMA", None)])
                )
            else:
...
def _split_on_ed10(token_sets):
    ...
    for token_set in token_sets:
        ...
        if (len(token_set) > 2) and (
            (token_set[0].type in ["UINT", "NZUINT"]) and (token_set[1].type == "ED10")
        ):
...
def _split_on_ed8(token_sets):
    ...
    for token_set in token_sets:
        ...
        elif (token_set[0].type in ["INT", "UINT", "NZUINT"]) and (
            token_set[1].type == "ED8"
        ):
            new_token_sets.append(token_set[:2])
            new_token_sets.append(token_set[2:])
        else:
...
def _read_quoted_string(tokens):
    ...
    if type_string != "QUOTED_STRING":
        raise InvalidFormat(
            "Token {0:s} has invalid neighbouring token".format(tokens[0])
        )
    ed = QuotedString()
