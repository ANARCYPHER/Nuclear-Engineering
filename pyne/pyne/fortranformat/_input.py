...
def input(eds, reversion_eds, records, num_vals=None):
    ...
    if (num_vals > num_out_eds) and (num_rev_out_eds == 0):
        raise ValueError(
            "Not enough output edit descriptors in reversion format to output %d values"
            % num_vals
        )
...
def read_integer(ed, state, record):
    ...
    try:
        ...
        if state["exception_on_fail"]:
            raise ValueError(
                "%s is not a valid input for one of integer, octal, hex or binary"
                % substr
            )
        else:
            return (None, state)
...
def read_logical(ed, state, record):
    ...
    if (substr == "") and (
        config.RET_UNWRITTEN_VARS_NONE or config.RET_WRITTEN_VARS_ONLY
    ):
        return (None, state)
...
def read_float(ed, state, record):
    ...
    try:
        ...
        if state["exception_on_fail"]:
            raise ValueError(
                "%s is not a valid input as for an E, ES, EN or D edit descriptor"
                % substr
            )
        else:
            return (None, state)
